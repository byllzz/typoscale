...
export function TokenOutput({ steps }: TokenOutputProps) {
  ...
  return (
    <div className={`rounded-xl overflow-hidden border flex flex-col transition-colors duration-300 h-full w-full ${
      ...
      {/* Format tabs Header - Enforced Non-Wrapping Row Configuration */}
      <div className={`flex items-center justify-between border-b transition-colors duration-300 flex-shrink-0 pl-1 pr-3 ${
        darkMode ? 'border-stone-800 bg-stone-950' : 'border-stone-200 bg-white'
      }`}>

        {/* Horizontal fluid swipe area for responsive screens */}
        <div className="flex-1 overflow-x-auto flex items-center min-w-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <div className="flex items-center">
            {FORMAT_TABS.map(tab => (
              <button
                key={tab.id}
                onClick={() => setOutputFormat(tab.id)}
                className={`flex items-center gap-1.5 px-3.5 sm:px-5 py-3 text-xs sm:text-sm font-semibold transition-colors border-b-2 whitespace-nowrap ${
                  outputFormat === tab.id
                    ? darkMode
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-1.5 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${
              copyAllCopied
                ...
                : darkMode
                  ? 'bg-stone-900 border border-stone-800 text-stone-300 hover:text-amber-400 hover:border-amber-500/30'
                  : 'bg-stone-50 border border-stone-200 text-stone-600 hover:text-amber-600 hover:border-amber-500/30'
            }`}
          >
            {copyAllCopied ? (
              <>
      ...
      {/* Code Display Canvas - Optimized Internal Spacing Bounds */}
      <div className="overflow-auto flex-1 min-h-0 text-[12px] sm:text-sm">
        <pre className={`p-3 sm:p-6 font-mono leading-relaxed m-0 transition-colors duration-300 ${
      ...
      {/* Mobile-Optimized Footer Ribbon */}
      <div className={`border-t px-4 py-2.5 flex items-center justify-between gap-4 transition-colors duration-300 flex-shrink-0 ${
        darkMode ? 'border-stone-800 bg-stone-950/40' : 'border-stone-200 bg-stone-50/50'
      }`}>
        <div className="flex items-center gap-3.5 text-[11px] font-mono">
          <span className={`flex items-center gap-1 ${darkMode ? 'text-stone-500' : 'text-stone-400'}`}>
            <Clipboard size={12} className="flex-shrink-0" />
            <span>{steps.length}s</span>
          </span>

          <span className={`flex items-center gap-1 ${darkMode ? 'text-stone-500' : 'text-stone-400'} hidden sm:flex`}>
            <TextIcon size={12} className="flex-shrink-0" />
            <span className="truncate max-w-[120px]">{displayFont}</span>
          </span>
