...
interface WelcomeNoteProps {
  /** Controlled by App.tsx via sessionStorage — do not manage internally */
  isOpen: boolean;
  onClose: () => void;
  darkMode: boolean;
}
...
export function WelcomeNote({ isOpen, onClose, darkMode }: WelcomeNoteProps) {
  ...
  const handleClose = () => {
    onClose();
    // sessionStorage flag is set in App.tsx — nothing to do here
  };
  ...
  return (
    <>
      {/* Backdrop — click dismisses and sets sessionStorage via onClose → App.tsx */}
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className={`flex-1 p-5 flex flex-col gap-2.5 border-b md:border-b-0 md:border-r ${divider}`}>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {[['MIT','License'],['v1.0','Latest'],['Zero','Dependencies'],['CSS/TW','Exports']].map(([v, l]) => (
